--Begin Tools.lua :)
local SUDO = 479893224 -- put Your ID here! <===
...
local function warning(msg)
	...
	local d = math.floor(expiretime / 86400) + 1
        if tonumber(d) == 1 and not is_sudo(msg) and is_mod(msg) then
			if lang then
				tdcli.sendMessage(msg.to.id, 0, 1, '`متبقي يوم واحد لالغاء تفعيل البوت تواصل مع المطور`', 1, 'md')
			else
...
if is_admin1(tonumber(data.id_)) then
   if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is already a_ *AdminBot*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ هو  مساعد في بوت مسبقا`", 0, "md")
      end
   end
	    table.insert(_config.admins, {tonumber(data.id_), user_name})
		save_config()
     if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is add to_ *AdminBot*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ تم اضافته الى قائمة المساعدين`", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
if not is_admin1(data.id_) then
   if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is Not_ *AdminBot*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ هو ليس مساعد في بوت مسبقا`", 0, "md")
      end
   end
		table.remove(_config.admins, nameid)
		save_config()
    if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is delete from_ *AdminBot*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ تم حذفه من قائمة المساعدين`", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
if already_sudo(tonumber(data.id_)) then
  if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is already a_ *Sudolist*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ هو مطور مسبقا`", 0, "md")
      end
   end
     ...
     reload_plugins(true)
  if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is add a_ *Sudolist*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ تم اضافته مطور`", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
user_name = check_markdown(data.first_name_)
   ...
   if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is not a_ *Sudolist*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ هو ليس قي قائمة مطور`", 0, "md")
      end
   end
     ...
     reload_plugins(true) 
   if not lang then
    return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 User_ "..user_name.."\n🆔 ID *"..data.id_.."*\n _is delete from_ *Sudolist*", 0, "md")    else     return tdcli.sendMessage(arg.chat_id, "", 0, "_👤 العضو_ "..user_name.." \n🆔 ايدي*"..data.id_.."* \n`✅ تم حذفه من قائمة مطور`", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
    ...
    if lang then
  return tdcli.sendMessage(data.chat_id_, "", 0, "_العضو غبر موجود_", 0, "md")
   else
...
if already_sudo(tonumber(data.id_)) then
  ...
  if not lang then
   ...
   if not lang then
    ...
    if lang then
  return tdcli.sendMessage(arg.chat_id, "", 0, "_العضو عير موجود_", 0, "md")
   else
...
local function pre_process(msg)
	if msg.to.type ~= 'pv' then
		...
		if gpst and not chex and msg.from.id ~= SUDO and not is_sudo(msg) then
			...
			if lang then
				tdcli.sendMessage(msg.to.id, msg.id_, 1, '_البوت مفعل ليوم واحد تواصل مع المطور لزيادة ايام التفعيل_', 1, 'md')
			else
		...
		if chex and not exd and msg.from.id ~= SUDO and not is_sudo(msg) then
			local text1 = 'تم انتهاء التفعيل \n\nID:  <code>'..msg.to.id..'</code>\n\nلحذف البوت من المجموعة\n\n/leave '..msg.to.id..'\nللدخول الى المجموعة:\n/jointo '..msg.to.id..'\n_________________\nللشحن تفعيل البوت عن بعد استخدم اوامر ادناه...\n\n<b>للتفعيل البوت في مجموعة مدة شهر:</b>\n/plan 1 '..msg.to.id..'\n\n<b>للتفعيل البوت في مجموعة مدة ثلاث اشهر:</b>\n/plan 2 '..msg.to.id..'\n\n<b>للتفعيل البوت لا محدود للمجموعة:</b>\n/plan 3 '..msg.to.id
			local text2 = '_تم انتهاء شحن تفعيل البوت في المجموعة تواصل مع مطور لاعادة التفعيل_'
...
if matches[1] == "مسح الكاش" and is_sudo(msg) then
  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
      ...
      username_ = matches[2]
		...
		if ((matches[1] == "config" and not Clang) or (matches[1] == "التكوين" and Clang)) and is_admin(msg) then
			return set_config(msg)
		end
if is_sudo(msg) then
   		if ((matches[1]:lower() == 'run' and not Clang) or (matches[1] == "تفعيل" and Clang)) and not redis:get('ExpireDate:'..msg.to.id) then
			redis:set('ExpireDate:'..msg.to.id,true)
			redis:setex('ExpireDate:'..msg.to.id, 180, true)
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '`تم شحن البوت تلقائيا 3 دقائق للضبط اعدادات`', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_Group charged 3 minutes  for settings._', 1, 'md')
				end
		end
		if ((matches[1] == 'off' and not Clang) or (matches[1] == "تعطيل" and Clang)) then
			if redis:get('CheckExpire::'..msg.to.id) then
				redis:del('CheckExpire::'..msg.to.id)
			end
			redis:del('ExpireDate:'..msg.to.id)
		end
		if ((matches[1]:lower() == 'gid' and not Clang) or (matches[1] == "قروب ايدي" and Clang)) then
			tdcli.sendMessage(msg.to.id, msg.id_, 1, '`'..msg.to.id..'`', 1,'md')
		end
		if ((matches[1] == 'leave' and not Clang) or (matches[1] == "طرد البوت" and Clang)) and matches[2] then
			if lang then
				tdcli.sendMessage(matches[2], 0, 1, 'تم طردي عن طريق المطور\nتواصل مع مطور للاستفسار', 1, 'md')
				tdcli.changeChatMemberStatus(matches[2], our_id, 'Left', dl_cb, nil)
				tdcli.sendMessage(SUDO, msg.id_, 1, 'البوت في المجموعة '..matches[2]..' تم اخراجه', 1,'md')
			else
		...
		if string.match(matches[2], '^-%d+$') then
			if tonumber(matches[3]) > 0 and tonumber(matches[3]) < 1001 then
				...
				if lang then
					tdcli.sendMessage(SUDO, 0, 1, 'تم شحن البوت في المجموعة '..matches[2]..' ايام التفعيل '..matches[3]..' يوما', 1, 'md')
					tdcli.sendMessage(matches[2], 0, 1, 'تم شحن تفعيل البوت في مجموعة مدة `'..matches[3]..'` يوما\nلمعرفة مدة متبقية للتفعيل ارسل\n /check',1 , 'md')
				else
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_يمكنك شحن مدة تفعيل من يوم الى 1000 يوم_', 1, 'md')
				else
		...
		if matches[1]:lower() == 'plan' or matches[1] == 'عرض' then
		...
		if string.match(matches[3], '^-%d+$') then
			...
			if lang then
				tdcli.sendMessage(SUDO, msg.id_, 1, '30 يوما مدة تفعيل للمجموعة '..matches[3]..' تم شحنها بنجاح', 1, 'md')
				tdcli.sendMessage(matches[3], 0, 1, '_تم شحن البوت في مجموعة مدة شهر_', 1, 'md')
			else
		...
		if string.match(matches[3], '^-%d+$') then
			...
			if lang then
				tdcli.sendMessage(SUDO, 0, 1, '90 يوما مدة تفعيل للمجموعة '..matches[3]..' تم شحنها بنجاح', 1, 'md')
				tdcli.sendMessage(matches[3], 0, 1, '_تم شحن البوت في مجموعة مدة ثلاث اشهر_', 1, 'md')
			else
		...
		if string.match(matches[3], '^-%d+$') then
			...
			if lang then
				tdcli.sendMessage(SUDO, msg.id_, 1, ' تفعيل لا محدود للمجموعة '..matches[3]..' تم شحنها بنجاح', 1, 'md')
				tdcli.sendMessage(matches[3], 0, 1, '_تم شحن البوت في مجموعة تفعيل لا محدود_', 1, 'md')
			else
		...
		if (matches[1]:lower() == 'jointo' or matches[1] == 'اضافتي الى') and matches[2] and is_admin(msg) then
		if string.match(matches[2], '^-%d+$') then
			if lang then
				tdcli.sendMessage(SUDO, msg.id_, 1, 'تم اضافتك الى المجموعة '..matches[2]..' ', 1, 'md')
				tdcli.addChatMember(matches[2], SUDO, 0, dl_cb, nil)
				tdcli.sendMessage(matches[2], 0, 1, '_تم دخول المطور_', 1, 'md')
			else
	...
	if msg.to.type == 'channel' or msg.to.type == 'chat' then
		if ((matches[1] == 'charge' and not Clang) or (matches[1] == "شحن" and Clang)) and matches[2] and not matches[3] and is_sudo(msg) then
			if tonumber(matches[2]) > 0 and tonumber(matches[2]) < 1001 then
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, 'تم شحن تفعيل البوت\nمدة تفعيل البوت هي *'..matches[2]..'* يوما', 1, 'md')
					tdcli.sendMessage(SUDO, 0, 1, 'تم شحن تفعيل البوت مدة '..matches[2]..' يوما في مجموعة `'..msg.to.id..'` ', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, 'تم شحن تفعيل البوت\nمدة تفعيل البوت هي *'..matches[2]..'* يوما', 1, 'md')
					tdcli.sendMessage(SUDO, 0, 1, 'تم شحن تفعيل البوت مدة '..matches[2]..' يوما في مجموعة `'..msg.to.id..'` ', 1, 'md')
				end
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_يمكنك شحن من يوم الى 1000 يوم_', 1, 'md')
				else
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_Expire days must be between 1 - 1000_', 1, 'md')
				end
			end
		end
	if (matches[1]:lower() == 'check' or matches[1] == 'تحقق') and is_mod(msg) and not matches[2] and is_owner(msg) then
			local expi = redis:ttl('ExpireDate:'..msg.to.id)
			if expi == -1 then
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '_البوت مفعل تفعيل لا محدود_', 1, 'md')
				else
				...
				if lang then
					tdcli.sendMessage(msg.to.id, msg.id_, 1, '`'..day..'` عدد ايام المتبقية للتفعيل البوت', 1, 'md')
				else
...
if ((matches[1] == 'import' and not Clang) or (matches[1] == "اضافة البوت" and Clang)) and is_admin(msg) then
...
return '`تم اضافة البوت للمجموعة`'
  end
  end
end

if ((matches[1] == 'setbotname' and not Clang) or (matches[1] == "تغيير اسم بوت" and Clang)) and is_sudo(msg) then
tdcli.changeName(matches[2])
   if not lang then
return '_Bot Name Changed To:_ *'..matches[2]..'*'
  else
return '_اسم البوت الجديد هو:_ \n*'..matches[2]..'*'
   end
end

if ((matches[1] == 'setbotusername' and not Clang) or (matches[1] == "تغيير معرف بوت" and Clang)) and is_sudo(msg) then
tdcli.changeUsername(matches[2])
   if not lang then
return '_Bot Username Changed To:_ @'..matches[2]
  else
return '_معرف بوت الجديد هو:_ \n@'..matches[2]..''
   end
end

if ((matches[1] == 'delbotusername' and not Clang) or (matches[1] == "حذف معرف بوت" and Clang)) and is_sudo(msg) then
tdcli.changeUsername('')
   if not lang then
return '*Done!*'
  else
return '`تم حذف المعرف للبوت`'
  end
end

if ((matches[1] == 'markread' and not Clang) or (matches[1] == "صحين" and Clang)) and is_sudo(msg) then
if ((matches[2] == 'on' and not Clang) or (matches[2] == "تفعيل" and Clang)) then
redis:set('markread','on')
   if not lang then
return '_Markread >_ *ON*'
else
return '`تم تفعيل وضع ظهور الصحين`'
   end
end
...
return '`تم الغاء وضع الصحين`'
      end
   end
end

if ((matches[1] == 'bc' and not Clang) or (matches[1] == "ارسال للمجموعة" and Clang)) and is_admin(msg) then
		local text = matches[2]
tdcli.sendMessage(matches[3], 0, 0, text, 0)	end

if ((matches[1] == 'broadcast' and not Clang) or (matches[1] == "ارسال للمجموعات" and Clang)) and is_sudo(msg) then		
local data = load_data(_config.moderation.data)		
local bc = matches[2]			
...
tdcli.sendMessage(k, 0, 0, bc, 0)			
  ...
  if is_sudo(msg) then
    ...
    if ((matches[1]:lower() == 'save' and not Clang) or (matches[1] == "حفظ" and Clang)) and matches[2] and is_sudo(msg) then
...
if ((matches[1] == 'sudolist' and not Clang) or (matches[1] == "قائمة المطورين" and Clang)) and is_sudo(msg) then
return sudolist(msg)
    end
if ((matches[1] == 'chats' and not Clang) or (matches[1] == "قائمة المجموعات" and Clang)) and is_admin(msg) then
return chat_list(msg)
    end
   if ((matches[1]:lower() == 'join' and not Clang) or (matches[1] == "اضافتي" and Clang)) and is_admin(msg) and matches[2] then
	   tdcli.sendMessage(msg.to.id, msg.id, 1, 'I Invite you in '..matches[2]..'', 1, 'html')
	   tdcli.sendMessage(matches[2], 0, 1, "تم دخول المطور🌚", 1, 'html')
    tdcli.addChatMember(matches[2], msg.from.id, 0, dl_cb, nil)
  end
		if ((matches[1] == 'off' and not Clang) or (matches[1] == "تعطيل" and Clang)) and matches[2] and is_admin(msg) then
    local data = load_data(_config.moderation.data)
			...
			save_data(_config.moderation.data, data)
	   tdcli.sendMessage(matches[2], 0, 1, "تم الغاء تفعيل البوت عن طريق المطور تواصله معه للاستفسار", 1, 'html')
    return '_المجموعة_ *'..matches[2]..'* _تم الغاء تفعيلها_'
		end
...
if ((matches[1] == 'adminlist' and not Clang) or (matches[1] == "قائمة المساعدين" and Clang)) and is_admin(msg) then
return adminlist(msg)
    end
     if ((matches[1] == 'leave' and not Clang) or (matches[1] == "خروج البوت" and Clang)) and is_admin(msg) then
  tdcli.changeChatMemberStatus(msg.to.id, our_id, 'Left', dl_cb, nil)
   end
     if ((matches[1] == 'autoleave' and not Clang) or (matches[1] == "خروج تلقائي" and Clang)) and is_admin(msg) then
local hash = 'auto_leave_bot'
--Enable Auto Leave
     if ((matches[2] == 'enable' and not Clang) or (matches[2] == "تفعيل" and Clang)) then
    redis:del(hash)
   return '`تم تفعيل الخروج تلقائي للبوت`'
--Disable Auto Leave
     elseif ((matches[2] == 'disable' and not Clang) or (matches[2] == "تعطيل" and Clang)) then
    redis:set(hash, true)
   return '`تم الغاء تفعيل الخروج تلقائي للبوت`'
--Auto Leave Status
      elseif ((matches[2] == 'status' and not Clang) or (matches[2] == "وضع خروج تلقائي" and Clang)) then
